...
void fillEBData(BMU_INFO *bm){
		...
		UNSIGNED8 batIdx = 0;
			...
			ebData[vdnIdx].maxContinousInpCurrent = 0; /* ??? */
			ebData[vdnIdx].maxContinousOutCurrent = 0; /* ??? */
			ebData[vdnIdx].maxVoltage = 58000l; /* ??? */
			ebData[vdnIdx].minVoltage = 42000l; /* ??? */
			ebData[vdnIdx].actualCurrent = bmu.m_curr;
			ebData[vdnIdx].actualVoltage = bmu.V_bmu;
			ebData[vdnIdx].electronicTemperature = 0; /* ??? */
			...
			ebBatData[batIdx].packRatedWhCapacity = 0; /* ?? */
			ebBatData[batIdx].temperature[0] = 0; /* ?? */
			ebBatData[batIdx].packMaximumCellVoltage = bmu.cv_max*10; /* ?? */
			ebBatData[batIdx].packMinimumCellVoltage = bmu.cv_min*10; /* ??? */
			ebBatData[batIdx].packActualWhCapacity = 0; /* ??? */
			ebBatData[batIdx].fullOutputWhCapacity = 44000; /* ??? */
			ebBatData[batIdx].relativeWhCapacity = 0; /* ??? */
			ebBatData[batIdx].packSOH = 10000ul; /* ??? */
			ebBatData[batIdx].Cell1 = (REAL32)bmu.cv[0]/1000; /* ??? */
			ebBatData[batIdx].Cell2 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell3 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell4 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell5 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell6 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell7 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell8 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell9 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell10 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell11 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell12 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell13 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell14 = (REAL32)bmu.cv[0]; /* ??? */
			ebBatData[batIdx].Cell15 = 0000ul; /* ??? */
			ebBatData[batIdx].BalCell1 = 1; /* ??? */
			ebBatData[batIdx].BalCell2 = 2; /* ??? */
			ebBatData[batIdx].BalCell3 = 3; /* ??? */
			...
			ebBatData[batIdx].BalCell14 = 0000ul; /* ??? */
			ebBatData[batIdx].BalCell15 = 0000ul; /* ??? */
			ebBatData[batIdx].packVoltage = bmu.V_bmu; /* 0x2004:1 */
			ebBatData[batIdx].fuseVoltage = 0; /* 0x2004:2 */
			ebBatData[batIdx].motVoltage = 222; /* 0x2004:3 */
			ebBatData[batIdx].chargeVoltage = 0; /* 0x2004:4 */
			ebBatData[batIdx].HVIPL0Voltage = 0; /* 0x2004:5 */
			ebBatData[batIdx].HVIPL1Voltage = 0; /* 0x2004:6 */
			ebBatData[batIdx].Temp1 = bmu.ct[0]; /* 0x2003:1 */
			ebBatData[batIdx].Temp2 = bmu.ct[1]; /* 0x2003:2 */
			ebBatData[batIdx].Temp3 = bmu.ct[2]; /* 0x2003:3 */
			ebBatData[batIdx].Temp4 = bmu.ct[3]; /* 0x2003:4 */
			ebBatData[batIdx].packCurrent = bmu.m_curr; /* 0x2005:1 */
			ebBatData[batIdx].Current_12V = 12345; /* 0x2005:2 */
			ebBatData[batIdx].preChargeDone=0; /* 0x2006:1 */
			ebBatData[batIdx].isChargerConnected=0; /* 0x2006:2 */
#endif
