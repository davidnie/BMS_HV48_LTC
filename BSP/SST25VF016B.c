...
void SPI_LTC_Init(void)
{
    ...
    ////SPI init Set PB13,14,15 as Output push-pull - SCK, MISO and MOSI
	...
	SPI_Init(SPI2, &SPI_InitStructure);   //init SPI2
	
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);   //init CS  GPIO clock
...
void SPI_FLASH_Init(void)
{
	...
	SPI_Init(SPI1, &SPI_InitStructure);   //set SPI1
...
*******************************************************************************/
	...
	//	GPIO_Init(GPIOB, &GPIO_InitStructure);
	  ...
	  GPIO_Init(GPIOD, &GPIO_InitStructure);
		...
		//SPI_Init(SPI3, &SPI_InitStructure);	//¸ù¾ÝSPI_InitStructÖÐÖ¸¶¨µÄ²ÎÊý³õÊ¼»¯ÍâÉèSPI2¼Ä´æÆ÷
	//	
	//	RCC_AHB1PeriphClockCmd(RCC_FLASH_CSN, ENABLE);	 //Ê¹ÄÜGPIOµÄÊ±ÖÓ
	//	   GPIO_InitStructure.GPIO_Pin = FLASH_CSN; 	 
	//	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	//	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	//	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	...
	//	SPI_FLASH_CS_HIGH();
		...
		//nie SPI_I2S_DMACmd(SPI2, SPI_I2S_DMAReq_Rx, ENABLE);
		//nie SPI_I2S_DMACmd(SPI2, SPI_I2S_DMAReq_Tx, ENABLE);
...
//	spi_1_flags.revFlag=1;
		unsigned char rev_data;
	
    if(SPI_I2S_FLAG_RXNE){
			rev_data=SPI2->DR;
			insertByteToBuffer((BUFFER *)&spi1RevBuf, rev_data);
	  }
